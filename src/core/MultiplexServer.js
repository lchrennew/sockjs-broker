    ...
    constructor(ws) {
        ...
        const multiplex = {
            'uns': (channels, topic, sub) => {
                this.logger.info(`Unsubscribe ${topic}`)
                delete channels[topic];
                sub.emit('close')
            },
            'msg': (channels, topic, sub, payload) => {
                this.logger.info(`Publish ${payload} --> ${topic}`)
            ...
            'sub': (channels, topic, sub) => {
                this.logger.info(`Subscribe ${topic}`)
                if (!(topic in this.registeredChannels)) {
                    this.logger.info(`Registering and connect to topic ${topic}`)
                    this.ws.emit('requireChannel', topic, sub)
                } else {
                    this.logger.info(`Connect to topic ${topic}`)
